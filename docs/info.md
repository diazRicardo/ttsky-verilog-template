...
No external hardware is required for basic operation. The ALU can be driven entirely through the TinyTapeout demo board's DIP switches and directly observed on the output header pins or an oscilloscope/logic analyzer.

For a practical demonstration, the 8 input DIP switches can provide operand data, and the output pins can drive LEDs to display the result and flags.
